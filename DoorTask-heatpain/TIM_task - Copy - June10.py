...
params = {
    ...
    'questionDownKey': '8',  # move slider left
    'questionUpKey': '9',  # move slider right
    'questionDur': 999.0,
    'vasStepSize': 1.0,  # how far the slider moves with a keypress (increase to move faster)
    ...
    'questionSelectKey': '0',  # select answer for VAS
...
# dlg = gui.DlgFromDict(expInfo, title=scriptName, order=['subject','session','Version','LHeat','MHeat','HHeat','skipPrompts','eyeLinkSupport','painSupport','paramsFile'])
...
dlg = gui.DlgFromDict(expInfo, title=scriptName,
                      order=['subject', 'session', 'LHeat', 'MHeat', 'HHeat', 'eyeLinkSupport', 'painSupport',
                             'skipPrompts', 'paramsFile'])
if not dlg.OK:
    core.quit()  # the user hit cancel, so exit
...
def PersistentScale(question, options, win, name='Question', textColor='black', pos=(0., 0.), stepSize=3.,
                    scaleTextPos=[0., 0.45],
    ...
    for iQ in range(nQuestions):
        ...
        markerStim = visual.ShapeStim(win, lineColor=textColor, fillColor=textColor, vertices=(
            (-markerSize / 2., markerSize * np.sqrt(5. / 4.)), (markerSize / 2., markerSize * np.sqrt(5. / 4.)),
            (0, 0)),
        ...
        ratingScale = visual.RatingScale(win, scale=question[iQ], \
                                         low=0., high=36., markerStart=0., precision=1., labels=options[iQ],
...
def MakePersistentVAS(question, options, win, name='Question', textColor='black', pos=(0., -0.70), stepSize=3.,
                      scaleTextPos=[0., -0.50],
                      labelYPos=-0.75, markerSize=0.1, tickHeight=0.0, tickLabelWidth=0.0,
                      downKey=params['questionDownKey'], upKey=params['questionUpKey'], selectKey=['q'],
                      hideMouse=True):
    # Make triangle
    markerStim = visual.ShapeStim(win, lineColor=textColor, fillColor=textColor, vertices=(
    ...
    ratingScale = visual.RatingScale(win, scale=question, \
                                     low=0., high=36., markerStart=0., precision=1., labels=options,
                                     tickMarks=tickMarks, tickHeight=tickHeight, \
                                     marker=markerStim, markerColor=textColor, markerExpansion=1, singleClick=False,
                                     disappear=False, \
                                     textSize=0.8, textColor=textColor, textFont='Helvetica Bold', showValue=False, \
