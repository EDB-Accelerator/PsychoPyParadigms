...
function vasPreRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of vasPreComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function vasPreRecordRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of vasPreRecordComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function practiceAnticipationRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of practiceAnticipationComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function practiceAwardRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of practiceAwardComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function practiceITIRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of practiceITIComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function fortuneWheelRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of fortuneWheelComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function fortuneWheelResultRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of fortuneWheelResultComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function doorImgShuffleRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of doorImgShuffleComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function doorStartScreenRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of doorStartScreenComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function doorInitRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of doorInitComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function doorRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of doorComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function doorAnticipationRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of doorAnticipationComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function awardRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of awardComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function noAwardRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of noAwardComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function restScreenRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of restScreenComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function vasRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of vasComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function vasRecordRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of vasRecordComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function afterVASRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of afterVASComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function finalRewardRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of finalRewardComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
