...
expName = 'door-heatpain-gui'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/jimmy/github/PsychoPyParadigms/DoorTask-heatpain-gui/door-heatpain-gui_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    image = visual.ImageStim(
        ...
        image='img/title.jpg', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1024, 768),
    ...
    for thisTrial_12 in trials_12:
        ...
        # create an object to store info about Routine titleScreen
        titleScreen = data.Routine(
            name='titleScreen',
            components=[key_resp, image],
        )
        titleScreen.status = NOT_STARTED
        ...
        key_resp.rt = []
        _key_resp_allKeys = []
        # store start times for titleScreen
        titleScreen.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        titleScreen.tStart = globalClock.getTime(format='float')
        titleScreen.status = STARTED
        thisExp.addData('titleScreen.started', titleScreen.tStart)
        titleScreen.maxDuration = None
        # keep track of which components have finished
        titleScreenComponents = titleScreen.components
        for thisComponent in titleScreen.components:
        ...
        titleScreen.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            ...
            if not continueRoutine:  # a component has requested a forced-end of Routine
                titleScreen.forceEnded = routineForceEnded = True
                break
        ...
        # store stop times for titleScreen
        titleScreen.tStop = globalClock.getTime(format='float')
        titleScreen.tStopRefresh = tThisFlipGlobal
        thisExp.addData('titleScreen.stopped', titleScreen.tStop)
        # check responses
        ...
        if key_resp.keys != None:  # we had a response
            ...
            trials_12.addData('key_resp.duration', key_resp.duration)
