...
# Updated 1/29/19 by DJ - added global escape ke y
...
# create text stimuli
message1 = visual.TextStim(win, pos=[-.75,+.5], wrapWidth=1.5, color=params['textColor'], alignHoriz='left', name='topMsg', text="aaa",units='norm')
message2 = visual.TextStim(win, pos=[-.75,-.5], wrapWidth=1.5, color=params['textColor'], alignHoriz='left', name='bottomMsg', text="bbb",units='norm')
...
[topPrompts_rating_scale,bottomPrompts_rating_scale] = BasicPromptTools.ParsePromptFile(params['RatingScalePromptFile'])
...
def RunVas(questions,options):
    ...
    [rating,decisionTime,choiceHistory] = RatingScales.ShowVAS(questions,options,win, questionDur=params['questionDur'], \
        ...
        isEndedByKeypress=params['questionSelectAdvances'],textColor=params['vasTextColor'],name='Vas',stepSize=params['vasStepSize'])
...
if params['doRest']:
    ...
    if not params['skipPrompts']:
        win.callOnFlip(SetPortData,data=params['codePrompt'])
        BasicPromptTools.RunPrompts(topPrompts_rating_scale,bottomPrompts_rating_scale,win,message1,message2,name='rating_scaleRestPrompt',ignoreKeys=[params['triggerKey']])
    ...
    if not params['skipPrompts']:
        win.callOnFlip(SetPortData,data=params['codePrompt'])
        BasicPromptTools.RunPrompts(topPrompts_rest,bottomPrompts_rest,win,message1,message2,name='restPrompt',ignoreKeys=[params['triggerKey'],'1','2','3','4'])
...
if params['doMovie']:
   ...
   # Display prompts, pre-movie VAS
    if not params['skipPrompts']:
        win.callOnFlip(SetPortData,data=params['codePrompt'])
        BasicPromptTools.RunPrompts(topPrompts_rating_scale,bottomPrompts_rating_scale,win,message1,message2,name='rating_scaleMoviePrompt',ignoreKeys=[params['triggerKey']])
    ...
    if not params['skipPrompts']:
        win.callOnFlip(SetPortData,data=params['codePrompt'])
        BasicPromptTools.RunPrompts(topPrompts_movie,bottomPrompts_movie,win,message1,message2,name='moviePrompt',ignoreKeys=[params['triggerKey'],'1','2','3','4'])
...
if params['doFinalScan']:
    ...
    if not params['skipPrompts']:
        win.callOnFlip(SetPortData,data=params['codePrompt'])
        BasicPromptTools.RunPrompts(topPrompts_prefinal,bottomPrompts_prefinal,win,message1,message2,name='prefinalPrompt',ignoreKeys=[params['triggerKey']])
